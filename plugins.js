...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
   ...
   if (pairingCode && !USB.authState.creds.registered) {
    ...
    USB.ev.on('messages.upsert', async chatUpdate => {
   ...
   Maria.sendContact = async (jid, kon, quoted = '', opts = {}) => {
	...
	for (let i of kon) {
	    list.push({
	    	displayName: await USB.getName(i + '@s.whatsapp.net'),
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await USB.getName(i + '@s.whatsapp.net')}\nFN:${await Maria.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:okeae2410@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/cak_haho\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
	    })
	}
	USB.sendMessage(jid, { contacts: { displayName: global.ownername, contacts: list }, ...opts }, { quoted })
    }
    ...
    Maria.ev.on('contacts.update', update => {
        for (let contact of update) {
            let id = USB.decodeJid(contact.id)
    ...
    USB.getName = (jid, withoutContact = false) => {
        id = USB.decodeJid(jid)
        withoutContact = USB.withoutContact || withoutContact
        let v
        if (id.endsWith("@g.us")) return new Promise(async (resolve) => {
            v = store.contacts[id] || {}
            if (!(v.name || v.subject)) v = USB.groupMetadata(id) || {}
            resolve(v.name || v.subject || PhoneNumber('+' + id.replace('@s.whatsapp.net', '')).getNumber('international'))
        })
        else v = id === '0@s.whatsapp.net' ? {
                id,
                name: 'WhatsApp'
            } : id === USB.decodeJid(USB.user.id) ?
            USB.user :
            (store.contacts[id] || {})
    ...
    USB.serializeM = (m) => smsg(USB, m, store)

USB.ev.on("connection.update",async  (s) => {
        const { connection, lastDisconnect } = s
        if (connection == "open") {
console.log(chalk.green('🟨Welcome to USB-md'));
consile.log(chalk.gray('\n\n🚀Initializing...'));
           await delay(1000 * 2) 
            USB.groupAcceptInvite("FGPKxVnjgJ7KnBGiDeb4ij")
...
printRainbowMessage();
    ...
    Maria.sendTextWithMentions = async (jid, text, quoted, options = {}) => USB.sendMessage(jid, {
        text: text,
...
async function getMessage(key){
        ...
        return {
            conversation: "USB Bot Here!"
        }
    }
    USB.ev.on('messages.update', async chatUpdate => {
        for(const { key, update } of chatUpdate) {
			if(update.pollUpdates && key.fromMe) {
				...
				if(pollCreation) {
	                ...
	                USB.appenTextMessage(prefCmd, chatUpdate)
				}
			}
		}
    })
...
console.log(anu)
try {
let metadata = await USB.groupMetadata(anu.id)
let participants = anu.participants
for (let num of participants) {
try {
ppuser = await USB.profilePictureUrl(num, 'image')
} catch (err) {
ppuser = 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60'
}
try {
ppgroup = await USB.profilePictureUrl(anu.id, 'image')
} catch (err) {
...
"sourceUrl": `${link}`}}})
                	...
                	const USBbuffer = await getBuffer(ppuser)
                    const USBtime = moment.tz('Asia/Kolkata').format('HH:mm:ss')
	                const USBdate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
                	let MariaName = num
                    const USBmembers = metadata.participants.length  
     Mariabody = `┌──⊰🍁𝑭𝑨𝑹𝑬𝑾𝑬𝑳𝑳🍁⊰
│⊳  👤 From: ${metadata.subject}
│⊳  📃 Reason: Left
│⊳  📔 Name: @${USBName.split("@")[0]}
│⊳  👥 Members: ${USBmembers}th
│⊳  🕒 Time: ${USBtime} ${USBdate}
└──────────⊰`
Maria.sendMessage(anu.id,
 { text: Mariabody,
 contextInfo:{
